...
export const Hero: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const pointLight1 = new THREE.PointLight(0x5ff3e8, 2.5, 50);
    ...
    const colors = [0x5ff3e8, 0x39d6cc, 0x7c3aed, 0xd2bbff, 0xffffff];
    ...
    const animate = (time: number) => {
      ...
      if (!prefersReducedMotion) {
        ...
        spheresGroup.rotation.y = targetX * 0.35;
        spheresGroup.rotation.x = -targetY * 0.35;

        const elapsed = time * 0.001;

        sphereInstances.forEach((sphere) => {
          sphere.position.y += Math.sin(elapsed + sphere.userData.floatOffset) * 0.008;
          sphere.position.add(sphere.userData.velocity);

          // Boundary bouncing
          if (Math.abs(sphere.position.x) > 22) sphere.userData.velocity.x *= -1;
          if (Math.abs(sphere.position.y) > 16) sphere.userData.velocity.y *= -1;
          if (Math.abs(sphere.position.z) > 12) sphere.userData.velocity.z *= -1;

          // Mouse proximity reaction
          const distToMouse = Math.sqrt(
            Math.pow(sphere.position.x / 18 - mouseX, 2) +
              Math.pow(sphere.position.y / 14 - mouseY, 2)
          );
          const scaleFactor = 1 + Math.max(0, (1 - distToMouse) * 0.4);
          sphere.scale.set(
            sphere.userData.baseScale * scaleFactor,
            sphere.userData.baseScale * scaleFactor,
            sphere.userData.baseScale * scaleFactor
          );
    ...
    <section className="relative min-h-screen flex flex-col justify-center items-center px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto pt-32 pb-20 overflow-hidden">
      ...
      <div className="relative z-10 text-center max-w-4xl mx-auto space-y-8">
        ...
        <p className="font-body text-lg sm:text-xl text-[#bbcac7] max-w-2xl mx-auto leading-relaxed">
          Where high-end digital craftsmanship meets rebellious creative energy. We design intuitive interfaces, build experimental 3D web applications, and engineer the future.
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
          ...
          >
            <span>Request to Join Club</span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-12 max-w-3xl mx-auto text-left">
          <div className="glass-card p-4 rounded-2xl border border-white/10">
            ...
            <p className="text-xs text-on-surface-variant">Active Domains</p>
          </div>
          <div className="glass-card p-4 rounded-2xl border border-white/10">
            <p className="font-display text-2xl font-bold text-secondary">50+</p>
            <p className="text-xs text-on-surface-variant">Core Creators</p>
          </div>
          <div className="glass-card p-4 rounded-2xl border border-white/10">
            <p className="font-display text-2xl font-bold text-[#ffd6ad]">24h</p>
            <p className="text-xs text-on-surface-variant">Annual Hackathon</p>
          </div>
          <div className="glass-card p-4 rounded-2xl border border-white/10">
            <p className="font-display text-2xl font-bold text-white">100%</p>
            <p className="text-xs text-on-surface-variant">Student Driven</p>
          </div>
        </div>
      </div>
    </section>
